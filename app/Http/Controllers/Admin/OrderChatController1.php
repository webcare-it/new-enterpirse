...
class OrderChatController extends Controller
{
    ...
    public function ask(Request $request)
    {
        ...
        try {
            ...
            if (in_array($lower, ['reset', 'cancel', 'start over', 'restart'])) {
                $result = $this->reply('reset', '🔄 Conversation reset. Product link paste korun.', ['state' => 'idle']);
            }
            // ---- STATE ROUTING ----
    ...
    /**
     * Start order flow for a product. $existingItems = already collected items in this order.
     */
    private function startOrderFlow(Product $product, array $existingItems = []): array
    {
        ...
        if ($product->is_variant && $product->variants->isNotEmpty()) {
            ...
            $html .= "<br>👉 Variant number likhun (jemon: <strong>1</strong>) ba <strong>cancel</strong> likhun.";
            ...
            return $this->reply('order_variant_list', $html, [
                ...
                'ctx'   => [
                    ...
                    'items'            => $existingItems, // ← carry forward
                ],
            ]);
        }
    ...
    private function askQuantity(int $productId, string $productName, array $ctx, string $priceLine = ''): array
    {
        ...
        $html .= "👉 <strong>Koyta</strong> order korte chan? Number likhun (jemon: <strong>2</strong>)";
    ...
    /**
     * Qty input → push item to ctx['items'], then ask "aro item?"
     */
    private function handleQtyInput(string $lower, array $ctx): array
    {
        if (in_array($lower, ['cancel', 'no', 'exit'])) {
            return $this->reply('order_cancelled', '❌ Order cancelled. Product link paste korun abar.', ['state' => 'idle']);
        }
        ...
        // Build new item and push to items array
        $items   = $ctx['items'] ?? [];
        $items[] = [
        ...
        // Show current cart + ask for more
        ...
        $html .= "<br>👉 <strong>Aro product kinte chan?</strong><br>";
        $html .= "Yes hole product link paste korun, na hole <strong>no</strong> likhun.";
    ...
    /**
     * Handle "more items?" — either new link (add) or "no" (proceed to shipping).
     */
    private function handleMoreItemsInput(string $raw, string $lower, array $ctx): array
    {
        ...
        // No / done → proceed to shipping
        if (in_array($lower, ['no', 'na', 'nope', 'done', 'finish', 'confirm', 'checkout'])) {
            $items = $ctx['items'] ?? [];
            if (empty($items)) {
                return $this->reply('order_empty', '⚠️ Kono item nai. Product link paste korun.', ['state' => 'idle']);
            }
            return $this->askShippingArea(['items' => $items]);
        }

        // Yes / link / product → try to detect product
        $product = $this->extractProductFromMessage($raw);

        if (!$product) {
            return $this->reply(
                'order_more_items',
                "⚠️ Product link ta detect korte parlam na. Abar paste korun, ba <strong>no</strong> likhun jodi ar kichu na chan.<br><br>"
                    . $this->renderCartHtml($ctx['items'] ?? []),
                ['state' => 'awaiting_more_items', 'ctx' => $ctx]
            );
        }

        // Continue adding — pass existing items forward
        return $this->startOrderFlow($product, $ctx['items'] ?? []);
    }

    /**
     * Ask shipping area (once for whole order).
     */
    private function askShippingArea(array $ctx): array
    {
        $items = $ctx['items'] ?? [];

        // If only 1 item and it has its own flat shipping cost → use it
        if (count($items) === 1) {
            $productShipping = $items[0]['product_shipping'] ?? null;
            if (
                is_array($productShipping)
                && ($productShipping['type'] ?? '') === 'flat_rate'
                && ($productShipping['cost'] ?? 0) > 0
            ) {
                $ctx['shipping_cost']       = (float) $productShipping['cost'];
                $ctx['shipping_area_id']    = null;
                $ctx['shipping_area_label'] = 'Product default (৳' . number_format((float) $productShipping['cost'], 2) . ')';

                $html  = "🚚 Shipping: product default cost <strong>৳" . number_format((float) $productShipping['cost'], 2) . "</strong><br><br>";
        ...
        // Otherwise, ask for area
        $areas = ShippingCost::orderBy('name')->limit(30)->get();

        if ($areas->isEmpty()) {
            $ctx['shipping_cost']       = 0;
            $ctx['shipping_area_id']    = null;
            $ctx['shipping_area_label'] = 'Free';

            $html  = "🚚 Kono shipping area configured nai. Shipping <strong>free</strong> dhora hocche.<br><br>";
            $html .= $this->infoPromptHtml($ctx);
        ...
        $html .= "<br>👉 Area number likhun (jemon: <strong>1</strong>) ba <strong>cancel</strong>.";
    ...
    private function handleShippingAreaInput(string $raw, array $ctx): array
    {
        ...
        if (!preg_match('/\d+/', $lower, $m)) {
            return $this->reply(
                ...
                "⚠️ Ekta area number likhun (jemon: <strong>1</strong>) ba <strong>cancel</strong>.",
        ...
        $ctx['shipping_cost']       = (float) $picked['cost'];
        $ctx['shipping_area_id']    = $picked['id'];
        $ctx['shipping_area_label'] = $picked['name'];

        $html  = "✅ Delivery: <strong>" . e($picked['name']) . "</strong> — ৳" . number_format((float) $picked['cost'], 2) . "<br><br>";
        $html .= $this->infoPromptHtml($ctx);
    ...
    /**
     * Render the current cart items block.
     */
    ...
    /**
     * Info prompt with full multi-item summary.
     */
    private function infoPromptHtml(array $ctx): string
    {
        ...
        $html .= "Ekhon ei 3 ta info din (comma diye):<br><br>";
        $html .= "<strong>name, phone, address</strong><br><br>";
        $html .= "Example: <code>Rahim, 01712345678, Mirpur Dhaka</code>";
    ...
    private function handleVariantSelection(string $lower, array $ctx): array
    {
        ...
        if (!preg_match('/\d+/', $lower, $m)) {
            return $this->reply(
                ...
                "⚠️ Ekta variant number likhun (jemon: <strong>1</strong>) ba <strong>cancel</strong>.",
    ...
    private function handleCustomerInfo(string $raw, array $ctx): array
    {
        ...
        if (count($parts) < 3) {
            return $this->reply(
                'order_info_invalid',
                "⚠️ 3 ta info lagbe: <strong>name, phone, address</strong><br>Example: <code>Rahim, 01712345678, Mirpur Dhaka</code>",
    ...
    /**
     * Create order with ALL items + one shipping.
     */
    private function createChatOrder(array $ctx, array $customer): Order
    {
        return DB::transaction(function () use ($ctx, $customer) {
            ...
            // Split shipping evenly across items? Or keep on first item only.
            // Standard practice: put shipping_cost on the ORDER only, set 0 on details.
            // But if your codebase expects it on OrderDetail, we put on first item.
            foreach ($items as $idx => $it) {
                ...
                OrderDetail::create([
                    ...
                    'price'                 => $price,
                    'tax'                   => 0,
                    'shipping_cost'         => $idx === 0 ? $shipCost : 0, // keep on first row
                    'quantity'              => $qty,
                    'payment_status'        => 'unpaid',
                    'delivery_status'       => 'pending',
                    'shipping_type'         => 'flat_rate',
    ...
    // ═══════════════════════════════════════════════════════════════
    // ANALYTICS ENGINE — SAME AS BEFORE (unchanged)
    // ═══════════════════════════════════════════════════════════════
    ...
    private function reply(string $intent, string $answer, array $meta = []): array
    {
        return [
            'intent' => $intent,
            'answer' => $answer,
