...
class VisitorLogMiddleware
{
    ...
    {
        if (function_exists('get_setting') && get_setting('visitor_counter') == 1) {
            $ip = $request->ip();
            $log = VisitorLog::where('ip_address', $ip)->first();
            ...
            if ($log) {
                ...
                // Increment visit count. You could add logic here to only increment once per session if desired.
                $log->increment('counts');
            } else {
                VisitorLog::create([
                    'ip_address' => $ip,
