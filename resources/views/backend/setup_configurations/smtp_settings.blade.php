        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('test.smtp') }}" method="post">
                    ...
                    <div class="row">
                        <div class="col">
                            <input type="email" class="form-control" name="email" value="{{ get_setting('order_notification_email') ?: auth()->user()->email }}" placeholder="{{ translate('Enter your email address') }}">
                        </div>
                        <div class="col-auto">
...
<h4 class="text-center text-muted mt-4">{{ translate('Email will be Used For') }}</h4>
<div class="row">
    <div class="col-lg-4">
        <div class="card">
            <div class="card-header">
                <h3 class="mb-0 h6">{{ translate('Order Placed Email (Customer)') }}</h3>
            </div>
            <div class="card-body text-center">
                <label class="aiz-switch aiz-switch-success mb-0">
                    <input type="checkbox" onchange="updateSettings(this, 'email_order_placed_customer')"
                        @if (get_setting('email_order_placed_customer') == 1) checked @endif>
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-header">
                <h3 class="mb-0 h6">{{ translate('Order Receive Email (Admin)') }}</h3>
            </div>
            <div class="card-body text-center">
                <label class="aiz-switch aiz-switch-success mb-0">
                    <input type="checkbox" onchange="updateSettings(this, 'email_order_placed_admin')"
                        @if (get_setting('email_order_placed_admin') == 1) checked @endif>
                    <span class="slider round"></span>
                </label>
            </div>
            <form action="{{ route('business_settings.update') }}" method="POST">
                @csrf
                <div class="px-4 pb-3">
                    <label class="form-label fw-semibold">{{ translate('Admin Email Address') }}</label>
                    <div class="input-group">
                        <span class="input-group-text"><i class="las la-envelope"></i></span>
                        <input type="hidden" name="types[]" value="order_notification_email">
                        <input type="email" name="order_notification_email" class="form-control" placeholder="admin@example.com"
                            value="{{ get_setting('order_notification_email') }}">
                    </div>
                </div>
